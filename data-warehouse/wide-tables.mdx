...
Our wide tables (also called One Big Table) are built from our dimensions and facts and can be thought of as flattened stars. We query the fact table selecting all columns and join onto all necessary dimensions. We recommend wide tables be used with BI platforms such as Google Looker Studio and Tableau. Below is a list of wide tables we have created in the past. We often start with these when building a new data warehouse and naturally move in different directions depending on the needs of the organization.
